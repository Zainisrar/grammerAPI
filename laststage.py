    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for mistake_word in mistakes:
                highlighted_sentence = re.sub(r'\b' + re.escape(mistake_word) + r'\b', f'<span>{mistake_word}</span>', highlighted_sentence)

            for mistake_word, correct_word in zip(mistakes, suggestions_2d_array):
                highlighted_sentence = re.sub(r'\b' + re.escape(mistake_word) + r'\b', f'<span class="highlight" data-corrected="{correct_word}">{mistake_word}</span>', highlighted_sentence)

            return render_template('newUI.html', original_sentence=highlighted_sentence, corrected_sentence=sentence,mistakes=mistakes)
